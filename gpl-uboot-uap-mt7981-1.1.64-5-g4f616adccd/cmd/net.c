...
int recovery_check(void)
{
	...
	if (!ubnt_flag("net_urescue"))
		return 0;

	retval = recovery_init();
	if (retval)
		return retval;
	return net_loop(RECOVERY);
}

int do_net_urescue(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	env_set("net_urescue", UBNT_FLAG_TRUE);
	return recovery_check() < 0 ? CMD_RET_FAILURE : CMD_RET_SUCCESS;
}

U_BOOT_CMD(
	net_urescue,     1,      1,      do_net_urescue,
	"force net_urescue", ""
);
