...
static void recovery_timeout(void)
{
	net_set_state(NETLOOP_FAIL);
}

static void recovery_handler(uchar *pkt, unsigned dport,
			  struct in_addr sip, unsigned sport,
			  unsigned len)
{
	recovery_t *rec = (void*)pkt;
	char s_ipaddr[24];
	uint8_t *ipaddr, *netmask;

	if (dport != RECOVERY_PORT)
		return;

	if (memcmp(UBNT_STR, rec->ubnt, sizeof(UBNT_STR) - 1) ||
		rec->ver != UBNT_REC_VERSION)
		return;

	if (rec->cmd != UBNT_REC_CMD_URESCUE)
		return;

	ipaddr = rec->u.urescue.ipaddr;
	netmask = rec->u.urescue.netmask;
	ipaddr[1] = (ipaddr[1] & netmask[1]) | (net_ethaddr[3] & ~netmask[1]);
	ipaddr[2] = (ipaddr[2] & netmask[2]) | (net_ethaddr[4] & ~netmask[2]);
	ipaddr[3] = (ipaddr[3] & netmask[3]) | (net_ethaddr[5] & ~netmask[3]);

	sprintf(s_ipaddr, "%d.%d.%d.%d", ipaddr[0], ipaddr[1], ipaddr[2], ipaddr[3]);
	printf("urescue: %s!\n", s_ipaddr);
	net_set_state(NETLOOP_SUCCESS);

	env_set("do_urescue", UBNT_FLAG_TRUE);
	env_set("do_net_urescue", UBNT_FLAG_TRUE);
	env_set("urescue_ip", s_ipaddr);
}

#define NET_URESCUE_TIMEOUT_DEFAULT 5
#define NET_URESCUE_TIMEOUT_MAX 30

static void recovery_start(void)
{
	uint32_t timeout = env_get_ulong("net_urescue_timeout", 10, NET_URESCUE_TIMEOUT_DEFAULT);
	if (timeout > NET_URESCUE_TIMEOUT_MAX)
		timeout = NET_URESCUE_TIMEOUT_MAX;

	printf("net_urescue: timeout=%d\n", timeout);
	timeout *= CONFIG_SYS_HZ;
	net_set_timeout_handler(timeout, recovery_timeout);
	net_set_udp_handler(recovery_handler);
}
#endif
