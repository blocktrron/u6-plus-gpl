...
#ifdef CONFIG_UBNT_APP
        if (ubnt_flag("appinitdone") && ((tftp_cur_block - 1) % 500) == 0) {
			 ...
			 unsigned src, unsigned len)
{
	...
	case TFTP_DATA:
		...
		if (tftp_state == STATE_SEND_RRQ) {
			...
			tftp_next_ack = tftp_windowsize;
		} else if (tftp_state == STATE_RECV_WRQ) {
			tftp_next_ack = tftp_cur_block;
			tftp_windowsize = 1;
		}
...
#ifdef CONFIG_UBNT_APP
static void ubnt_send_beacon(void)
{
	static uint8_t beacon_buf[256];
	static uint8_t beacon_len = 0;
	static struct in_addr beacon_ip = {.s_addr = 0x00000000};
	struct in_addr bcast_ip = {.s_addr = 0xffffffff};
	uint8_t bcast_mac[6] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
	if (!beacon_len || beacon_ip.s_addr != net_ip.s_addr) {
		beacon_len = ubnt_prepare_beacon(beacon_buf);
		beacon_ip = net_ip;
	}

	if (beacon_len) {
		uint8_t *pkt = net_tx_packet + net_eth_hdr_size() + IP_UDP_HDR_SIZE;
		memcpy(pkt, beacon_buf, beacon_len);
		net_send_udp_packet(bcast_mac, bcast_ip, 10001, 10000, beacon_len);
	}
}
#endif
...
#ifdef CONFIG_UBNT_APP
		ubnt_send_beacon();
		if (ubnt_flag("appinitdone") && tftp_state == STATE_RECV_WRQ) {
