...
static void mtk_dump_sgmii(void *_eth, u32 id, u32 offset, u32 range)
{
	struct mtk_eth *eth = _eth;
	u32 cur = offset;
	u32 val, val1, val2, val3;

	if (!eth->sgmii->regmap[id])
		return;

	pr_info("\n============ SGMII%d ============\n", id);
	while(cur < offset + range) {
		regmap_read(eth->sgmii->regmap[id], cur, &val);
		regmap_read(eth->sgmii->regmap[id], cur + 0x4, &val1);
		regmap_read(eth->sgmii->regmap[id], cur + 0x8, &val2);
		regmap_read(eth->sgmii->regmap[id], cur + 0xc, &val3);
		pr_info("0x%x: %08x %08x %08x %08x\n",
			cur,  val, val1, val2, val3);
		cur += 0x10;
	}
}
...
void mtk_dump_netsys_info(void *_eth)
{
	...
	mtk_dump_reg(eth, "GMAC", 0x10000, 0x240);
	mtk_dump_reg(eth, "MIB(GDM1)", 0x1c00, 0x40);
	mtk_dump_reg(eth, "MIB(GDM2)", 0x1c40, 0x40);
	mtk_dump_sgmii(eth, 0, 0x0, 0x200);
	mtk_dump_sgmii(eth, 1, 0x0, 0x200);
}
