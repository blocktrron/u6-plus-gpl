...
static uint16_t ubnthal_systemid = 0;
...
static void write_msg(struct console *con, const char *msg, unsigned int len)
{
	...
	if (!ubnthal_systemid) {
		void *sym;
		if ((sym = __symbol_get("ubnthal_get_systemid"))) {
			uint16_t (*ubnthal_get_systemid)(void) = (void *)sym;
			ubnthal_systemid = ubnthal_get_systemid();
		}
	}
	...
	if (len <= sizeof(netconsole_msg) - 16) {
		len = sprintf(netconsole_msg, "{%04x %04x} %.*s", random_id,
				ubnthal_systemid, len, msg);
