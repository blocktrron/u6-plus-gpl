				  ...
				  struct sk_buff *))
{
    ...
    if ((indev != NULL && UBNT_IS_NFBYPASS_ENABLE(indev)) ||
        (outdev != NULL && UBNT_IS_NFBYPASS_ENABLE(outdev))) {
        if (hook != NF_INET_PRE_ROUTING &&
            hook != NF_INET_POST_ROUTING) {
            return okfn(net, sk, skb);
        } else {
            if ((indev != NULL && UBNT_IS_NFBYPASS_MARK(indev)) ||
                (outdev != NULL && UBNT_IS_NFBYPASS_MARK(outdev)))
                return okfn(net, sk, skb);
